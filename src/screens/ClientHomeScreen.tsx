...
export default function ClientHomeScreen() {
  ...
  const secondaryActions = [
    { id: 'rights', title: 'اعرف حقوقك', icon: <BookOpen size={24} />, path: '/app/rights' },
    { id: 'contracts', title: 'صياغة عقود', icon: <FileText size={24} />, path: '/app/contracts' },
    { id: 'cases', title: 'قضاياي', icon: <Briefcase size={24} />, path: '/app/cases' },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: COLORS.background }}>
      ...
      <main className="flex-1 p-6 space-y-8 max-w-4xl mx-auto w-full">
        {/* Welcome Section */}
        <section>
          <h2 className="text-2xl font-bold mb-2" style={{ color: COLORS.text }}>أهلاً بك{userName ? `، ${userName}` : ''}</h2>
          <p className="text-sm" style={{ color: COLORS.muted }}>كيف يمكن لمحامينا مساعدتك اليوم؟</p>
        </section>
        ...
        <section>
          <h3 className="text-lg font-bold mb-4" style={{ color: COLORS.text }}>خدمات إضافية</h3>
        ...
        <section className="bg-white p-6 rounded-3xl border-2 border-dashed flex items-center justify-between" style={{ borderColor: COLORS.emergency }}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-bold" style={{ color: COLORS.emergency }}>حالة طوارئ قانونية؟</h3>
              <p className="text-xs" style={{ color: COLORS.muted }}>تواصل مع محامي طوارئ فوراً</p>
            </div>
          </div>
          <button className="px-6 py-2 rounded-xl text-white font-bold text-sm" style={{ backgroundColor: COLORS.emergency }}>
            اتصل الآن
          </button>
        </section>
      </main>
